...
export interface ListMenuElementsProps {
	menuElements: { name: string; id: string; onClick?: () => void; color?: string }[];
	selectedId: string;
	className?: string;
}
	...
	return (
		...
		<div className={`${className} flex-row flex gap-28`}>
			{menuElements.length > 0 &&
				menuElements.map((val, ind) => <MenuElement color={val.color} title={val.name} onClicked={val.onClick} selected={selectedId === val.id} key={`menuElement${ind}`} />)}
		</div>
		// </MenuContainer>
	);
};
